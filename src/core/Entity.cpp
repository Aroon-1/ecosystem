...
// empeche les entites de sortir de la zone 
Vector2D Entity::StayInBounds(float worldWidth, float worldHeight)
const{
    Vector2D p=position; // je copie la valeur de la position dans p, puisque l'attribut position reste constante.
    if(p.x < worldWidth-worldWidth) p.x=0;
    if(p.x > worldWidth) p.x =worldWidth;
    if(p.y <worldHeight-worldHeight) p.y =0;
...
if(foodSources.size()==0) return {0, 0}; // si il y aucune nourriture, on retourne un deplacement null
